    ...
    <div data-options="region:'center'">
        <div id="supplier_tt" class="easyui-tabs" data-options="fit:true" style="width:500px;height:250px;">
            <div title="首页" style="padding:20px;display:none;">
                <a id="wddd" class="easyui-linkbutton" data-options="iconCls:'icon-edit'">我的订单</a>
            </div>
        </div>
    </div>
</div>
<script>
    $("#supplier_tree").tree({
        onClick: function () {
            //判断是否打开了
            var res = $("#supplier_tt").tabs('exists', "查看商品");
            if (res) {
                //打开  选中已打开
                $("#supplier_tt").tabs('select', "查看商品");
            } else {
                //未打开  添加选项卡
                $("#supplier_tt").tabs('add', {
                    title: "查看商品",
                    closable: true,
                    href: "salesMain.action?url=salesCommodity"
                });
            }
        }
    })
    $("#wddd").click(function () {
        //判断是否打开了
        var res = $("#supplier_tt").tabs('exists', "我的订单");
        if (res) {
            //打开  选中已打开
            $("#supplier_tt").tabs('select', "我的订单");
        } else {
            //未打开  添加选项卡
            $("#supplier_tt").tabs('add', {title: "我的订单", closable: true, href: "salesMain.action?url=mySalesCommodity"});
        }
    })
